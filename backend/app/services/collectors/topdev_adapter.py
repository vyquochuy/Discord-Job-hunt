...
logger = logging.getLogger("topdev_adapter")


class TopDevJobCollector(BaseJobCollector):
    """
    Adapter thu thập tin tuyển dụng CNTT từ TopDev.vn.
    Hỗ trợ bóc tách danh mục IT Jobs, xử lý thẻ công nghệ, mức lương và địa điểm.
    """

    BASE_URL = "https://topdev.vn"
    SEARCH_URL = "https://topdev.vn/it-jobs"
            ...
            async with httpx.AsyncClient(timeout=15.0, follow_redirects=True) as client:
                while page <= max_pages and len(results) < limit:
                    ...
                    if response.status_code != 200:
                        logger.warning(f"TopDev page {page} returned status {response.status_code}")
                        break

                    soup = BeautifulSoup(response.text, "html.parser")
                    job_cards = soup.select(
                        ".job-item, .job-card, .job-item-info, div[data-job-id], article.item, div.item"
                    )
                ...
                logger.info(f"Successfully scraped {len(results)} IT jobs across {page-1} pages from TopDev.vn")
        except Exception as e:
            logger.error(f"Error scraping TopDev: {e}", exc_info=True)
