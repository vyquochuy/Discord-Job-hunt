...
class CareerLinkJobCollector(BaseJobCollector):
    ...
    Hỗ trợ danh mục IT - Phần mềm (Category 19) và tìm kiếm từ khóa.
    """
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0, follow_redirects=True) as client:
                ...
                if response.status_code != 200:
                    logger.warning(f"CareerLink returned status {response.status_code}")
                    return results
                ...
                logger.info(f"Successfully scraped {len(results)} jobs from CareerLink.vn")
